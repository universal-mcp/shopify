...
def app_instance():
    ...
    return ShopifyApp(integration=mock_integration)

def test_universal_mcp_shopify_app_initialization(app_instance):
    """
    Test that the ShopifyApp instance is initialized correctly with a name.
    """
...
def test_universal_mcp_shopify_tool_docstrings_format(app_instance):
    """
    Test that each tool method in ShopifyApp has a well-formatted docstring,
...
def test_universal_mcp_shopify_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in ShopifyApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
